...
static cgiParseResultType cgiXMLRPC();
...
int main(int argc, char *argv[]) {
	...
	if (cgiStrEqNc(cgiRequestMethod, "post")) {
		...
		if (cgiStrEqNc(cgiContentType, "application/x-www-form-urlencoded")) {	
			...
			CGICDEBUGEND	
#endif /* CGICDEBUG */
		} else if (cgiStrEqNc(cgiContentType, "text/xml")) {
#ifdef CGICDEBUG
			CGICDEBUGSTART
			fprintf(dout, "Calling XML-RPC\n");
			CGICDEBUGEND	
#endif /* CGICDEBUG */
			if (cgiXMLRPC() != cgiParseSuccess) {
#ifdef CGICDEBUG
				CGICDEBUGSTART
				fprintf(dout, "XML-RPC  failed\n");
				CGICDEBUGEND	
#endif /* CGICDEBUG */
				cgiFreeResources();
				return -1;
...
#ifdef CGICDEBUG
			CGICDEBUGSTART
			fprintf(dout, "XML-RPC succeeded\n");
			CGICDEBUGEND	
#endif /* CGICDEBUG */
		}
...
#ifdef CGICDEBUG
			CGICDEBUGSTART
			fprintf(dout, "GetFormInput succeeded %s\n",cgiQueryString);
			CGICDEBUGEND	
#endif /* CGICDEBUG */
		}
...
static cgiParseResultType cgiXMLRPC() {
	...
	value = (char *) malloc(cgiContentLength);
	if (!value) {
		return cgiParseMemory;	
	}
	...
	if (((int) fread(value, 1, cgiContentLength, cgiIn)) 		!= cgiContentLength) 
	{
		return cgiParseIO;
	}	

	value[cgiContentLength]=0;
...
cgiFormResultType cgiFormCookie (char *name, char *result, int max)
{
	char *tag,*val,*tmp;char *i;int flag,slen;
	tmp = cgiCookie;slen = strlen (name);
	while (1) {
		tag = strchr (tmp,'=');
		if (!tag) break;
		flag=1;
		if (slen!=(tag-tmp)) { flag=0; } else
		    for (i=tmp;(i<tag)&&((int)(i-tmp)<slen);i++)
			if (name[i-tmp]!=i[0]) flag=0;
		val = tag+1;
		tag = strchr (val,';');
		if (tag) { 
		    tmp = tag+2;
		} else if (flag)tag = strchr (val,'\0');
		if (tag) { 
		    if (flag) {
			for (i=val;(i<tag)&&(i-val<max-1);i++) result[i-val] = i[0];
			result[i-val] = '\0';
			return cgiFormSuccess;
		    }
		} else { result[0] = '\0';return cgiFormNotFound; }
	}
	result[0] = '\0';
